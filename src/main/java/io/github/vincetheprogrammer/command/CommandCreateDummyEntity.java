...
public class CommandCreateDummyEntity {
    ...
    private static int run(CommandContext<ServerCommandSource> commandContext) throws CommandSyntaxException {
        ...
        DummyPlayerEntity dummy = new DummyPlayerEntity(ImmersiveLogoff.DUMMY_PLAYER, world);
        dummy.setPos(blockPos.getX(), blockPos.getY(), blockPos.getZ());

        world.spawnEntity(dummy);

        dummy.setYaw(yaw);
        dummy.setPitch(pitch);
